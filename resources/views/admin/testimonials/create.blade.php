...
@section('content')
<div style="margin-bottom:1.5rem;"><a href="{{ route('admin.testimonials.index') }}" style="color:var(--green);font-size:.85rem;">← Back</a><h1 style="font-size:1.5rem;font-family:var(--font-head);margin-top:.5rem;">Add Testimonial</h1></div>
...
<form action="{{ route('admin.testimonials.store') }}" method="POST" class="aform">
    @csrf
    <div class="fg"><label>Quote *</label><textarea name="quote" required>{{ old('quote') }}</textarea></div>
    <div class="row2">
        <div class="fg"><label>Author Name *</label><input type="text" name="author_name" value="{{ old('author_name') }}" required></div>
        <div class="fg"><label>Location</label><input type="text" name="author_location" value="{{ old('author_location') }}" placeholder="e.g. Hanang'"></div>
    </div>
    <div class="row2">
